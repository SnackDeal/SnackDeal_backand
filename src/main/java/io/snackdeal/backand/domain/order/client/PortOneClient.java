 ...
 * V2 는 토큰 발급 없이 API Secret 을 Authorization 헤더로 바로 사용하며, 결제는 paymentId(주문번호)로 식별한다.
 * 서비스는 이 인터페이스에만 의존하므로, 단위 테스트에서는 구현체 대신 Mock 을 주입한다.
 */
public interface PortOneClient {

    // paymentId 로 실제 결제 내역을 조회한다. (GET https://api.portone.io/payments/{paymentId})
    PortOnePayment getPayment(String paymentId);

    // 결제를 취소한다. 금액 위변조 감지 등으로 확정 전 되돌릴 때 호출. (POST /payments/{paymentId}/cancel)
