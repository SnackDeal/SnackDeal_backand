...
public class EmailVerificationService {
    ...
    /*
     * 이메일 인증코드 발송.
     * 회원가입 전 단계이므로, 아래 두 가지를 먼저 막고 발송한다.
     ...
     * 성공하면 회원가입 API 에서 사용할 인증 토큰(verification_token)을 발급하고 유효시간(초)을 함께 반환한다.
     * → 이후 join 시 이 토큰으로 "이 이메일은 인증됐다"는 것을 증명한다. (getVerifiedEmail 참고)
     */
    @Transactional
