...
public interface AdminMemberApiDocs {
    ...
    @Operation(summary = "회원 리스트 조회",
            description = "keyword(이메일/이름 부분검색)와 status 필터로 회원을 조회한다. 페이지네이션(page, size) 지원.")
    @ApiResponses({
    ...
    @Operation(summary = "회원 상세 조회",
            description = "회원 id 로 기본 정보를 조회한다.")
    @ApiResponses({
    ...
    @Operation(summary = "회원 상태 변경",
            description = "회원 상태를 ACTIVE/INACTIVE/DELETED 로 변경한다. 하드 삭제하지 않고 상태+deleted_at 으로 처리한다. "
                    + "본인 계정은 변경 불가(403), 이미 탈퇴한 회원은 되돌릴 수 없다(422). DELETED 전환 시 세션(토큰)이 즉시 무효화된다.")
    @ApiResponses({
